...
## Phase H conversion: sixth slice, authenticate_session, and the callback resolver's real blocker (2026-09-11)

Two things closed out in this slice: a conversion that should have happened earlier in this arc, and a definitive answer on the one remaining control function.

**Converted, `postgres_sessions.py`'s `authenticate_session`**: this was one of the 13 confirmed Phase F control functions from the very first classification pass, but got missed while this conversion arc focused on `postgres_identity.py`, `postgres_jobs.py`, and `postgres_schedules.py`. Same shape as `authenticate_token`: resolves under `api_tenant_data` via `webguard_control.resolve_browser_session` (no tenant known from a bare session id/secret), then reads the three fields the resolver omits (`issued_at`, `user_agent`, `ip_address`) and runs the `last_used_at`/`idle_expires_at` write, both as ordinary tenant-scoped queries once `organization_id` is known. `browser_sessions` has a full table grant to `api_tenant_data` (like `principals`/`organizations`), so this converted cleanly with no ACL wall to work around. `revoked_at` on the returned record is always `None` in the success path: the method already raises `session_expired` before construction if the resolved row was revoked, matching the original code's own `is_usable()` check exactly. `tests/integration/test_postgres_sessions.py` already had real Postgres coverage calling this method directly (9 tests: creation, touch, CSRF mismatch, absolute expiry, revocation); it needed the same bootstrap fixture the other test files in this arc already got, and all 9 passed unmodified.

**`postgres_callback_service.py`'s `record_observation`, confirmed genuinely blocked, not a wiring gap**: unlike every other gap this phase found, `webguard_control.resolve_and_record_callback_observation` has no capability role granted to call it through at all. Its own SQL comment (Section 9) states this explicitly: the public callback receiver has no `organization_id` to scope by until a token resolves, so it cannot run as any of the three tenant-data roles (each already carries ordinary tenant-scoped privileges this pre-authentication path has no business holding), and `callback_function_owner` is the function's own privileged `SECURITY DEFINER` owner, not a caller-facing identity. Phase F's own review deliberately granted `EXECUTE` to no role at all rather than invent a new `NOLOGIN` role unreviewed, and explicitly named this as deferred to "the future deployment phase that actually wires a LOGIN identity to this path." Verified by reading the bootstrap SQL directly: a single `REVOKE ALL ... FROM PUBLIC` with no `GRANT EXECUTE` anywhere for this function, the only one of the 13 control functions with this property. Documented in the method's own docstring rather than left to rediscover.

**Proven against real disposable Postgres**: full contract suite (63/63), full Postgres integration sequence including the session tests and the `build_production_components` E2E, and the full 1799-test unit suite.

**This closes the "wire existing Phase F control functions" half of Phase H entirely.** Final accounting across all 13 functions: 9 converted and proven (`resolve_api_token`, `resolve_browser_session`, `resolve_principal_by_email`, `claim_next_job`, `renew_lease`, `recover_expired_leases`, `terminal_transition`, `list_due_schedules`, `block_due_schedule`); 3 exist and are correct but cannot be reached by their Python method's full contract without either a new SQL design decision or a narrower method contract (`resolve_identity_token`/`consume_identity_token`, `resolve_job_organization`/`get_scope`, `enqueue_due_schedule`); 1 has no callable path at all pending an explicit architecture decision Phase F's own reviewers already deferred (`resolve_and_record_callback_observation`/`record_observation`). What remains of Phase H is the larger, not-yet-started half: adding tenant-context-setting to the roughly 90 genuinely-ordinary methods across all 13 repository files, and the eventual RLS+FORCE activation in a real environment.

## Phase H conversion: tenant-context-setting begins, postgres_targets.py (2026-09-11)

Starts the second, larger half of Phase H: adding tenant-context-setting to the roughly 90 genuinely-ordinary methods across all 13 repository files (as distinct from the "wire existing control functions" half just closed out). `postgres_targets.py` was chosen as the first file since Phase H's own classification pass already confirmed it as the simplest possible case: 6 methods, 0 exceptions, every one already taking `organization_id` and using it correctly in its own SQL predicate.

**New pool primitive**: `postgres_pool.py`'s `tenant_connection` gained an optional `role` parameter. Previously it only set the transaction-local tenant-context GUC; now, when `role` is given, it runs `SET LOCAL ROLE` first (validated against the same three-value set `role_scoped_connection` validates against), then sets the GUC on that now-restricted connection. This is the one primitive an ordinary method needs: unlike `authenticate_token`/`authenticate_session`, `organization_id` is already a parameter, so there is no pre-auth resolve step, just "run this already-correctly-scoped query under the restricted role, with tenant context set for when RLS activates." Fully backward compatible: every existing `tenant_connection` caller (only test code today) that doesn't pass `role` gets the exact prior behavior.

All 6 `postgres_targets.py` methods now call `self._pool.tenant_connection(organization_id, role=API_TENANT_DATA_ROLE)` instead of the bare `self._pool.connection()`. `targets` has no Phase F control-function counterpart because none is needed: `api_tenant_data` already has full `SELECT, INSERT, UPDATE` on the table.

**New test coverage**: added `TenantConnectionRoleTests` to `tests/integration/test_postgres_connection_pool.py` (3 tests: the role switch and GUC both take effect together, the role switch does not outlive its own transaction, an unknown role is rejected before ever borrowing a connection), and added the tenant-isolation bootstrap to `tests/contract/test_target_repository_contract.py`'s Postgres fixture, which had none before since nothing in that file previously depended on any role existing.
...
## Phase H conversion: postgres_callback_service.py and postgres_callback_broker.py, the arc's last file (2026-09-12)

Converts the ordinary methods in `postgres_callback_service.py` and `postgres_callback_broker.py`, the final two files in the tenant-context-setting arc. `record_observation` stays untouched, still the documented, genuinely blocked gap from an earlier PR (Phase F granted `webguard_control.resolve_and_record_callback_observation` EXECUTE to no role at all, deliberately, pending a future pre-auth identity design).

The real finding here: unlike every other file converted in this arc, `callback_registrations` and `callback_observations` grant `worker_tenant_data`, not `api_tenant_data`, `SELECT`/`INSERT`. Tracing actual callers explains why: `production_startup.py` wires `PostgresCallbackRegistrationRepository` only into `postgres_callback_broker.PostgresCallbackBroker`, which is itself wired only into `executor.py`'s `ScanJobExecutor`, the worker. `register` and `get_registration` (both have a real `organization_id`) now run under `worker_tenant_data` via `tenant_connection`. `postgres_callback_broker.py`'s own `_latest_observation` (no `organization_id`, polled by `wait_for_observation`, worker-only) runs under `worker_tenant_data` via `role_scoped_connection`.

`revoke_registration` is a distinct case from the worker/scheduler-only gaps found in the previous three PRs: it has a real `organization_id`, but `tenant_isolation_acl.sql`'s own header comment already records that this method (and its broker wrapper) has zero live callers anywhere in this codebase, so `callback_registrations` was deliberately never granted `UPDATE` for any tenant-data role. No role exists that could run this method's `UPDATE` today. Stays on the unrestricted connection with a docstring citing the ACL file's own reasoning directly.

Two test fixes were needed, both because this file's role choice differs from every prior one in the arc: `tests/unit/test_postgres_callback_broker.py`'s `_FakePool` only mimicked `.connection()`; added a `role_scoped_connection()` method that delegates to the same fake logic, since `_latest_observation` now calls it. `tests/contract/test_callback_registration_repository_contract.py`'s Postgres-backed test class had no tenant-isolation bootstrap at all; added the standard fixture.
...
## Platform expansion: SOC and Compliance, reconciliation and ledger setup (2026-09-12)

The owner supplied `docs/audit/OPENHUNTX_THREE_MODULE_PLATFORM_HANDOFF_2026-09.md` (committed verbatim this PR for durability; it previously existed only on the owner's Desktop, outside version control) and instructed building OpenHuntX SOC and OpenHuntX Compliance alongside WebGuard, "side by side." Full document read (677 lines, sections 1-24, complete source disposition register for 53 SOC and 84 Compliance source sections).

Reconciliation against this repository's own actual state (the handoff's own reported baseline, commit `da5da85`/Phase H "not started"/Open P1 total 6, predates this session's own Phase H and Coverage Truth Map work and is stale): current `origin/main` is `922cb31`. Phase H is complete (PRs #30-47). P1-6 and P1-7 are closed (PRs #17, #19). Open P1 total is 4 (P1-2, P1-8, P1-9, P1-12-R1), not 6. Coverage Truth Map v1 shipped (PRs #49-50). No scope conflict was found in this repository's own tracked files against the handoff's expansion decision; the handoff's own claim that an earlier "avoid building SOC/Compliance" instruction is superseded is taken as given per the owner's direct instruction, not independently re-derived from a source this session has access to.
