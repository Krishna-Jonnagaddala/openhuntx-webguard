...
**Next steps (Slice 21 addendum):** built. `active.xxe.disclosure` (`xxe_disclosure_detector.py`) is the in-band variant proposed here, using exactly the signature-based classification this line called for. It closes the general in-band-vs-out-of-band gap for any endpoint this detector's shared candidate discovery already reaches, but it does **not** reconfirm the specific Juice Shop `/file-upload` finding: that vulnerability is a genuine `multipart/form-data` upload, a request shape neither this detector nor `active.xxe.callback` can construct, and one this scanner's attack-surface discovery cannot even represent (no FILE/MULTIPART `InputLocation` exists). See `docs/audit/active-detection-phase18-xxe-disclosure.md` for the full account of what was, and was not, closed.
