...
**Not done this slice.** `_apply_xxe_callback_detection` is wired into `executor.py`'s call sequence but has no dedicated integration/unit test exercising that wiring, the same gap `_apply_ssrf_callback_detection` itself has at the executor layer (its own coverage lives in `tests/integration/test_ssrf_callback_e2e_lab.py` and `tests/integration/test_production_ssrf_callback_e2e.py`, both opt-in and both specific to SSRF).
