...
- No true end-to-end test exists yet, the same gap path traversal, command injection, XXE, and open redirect all have.
- The real-network validation added in Slice 18 depends on the optional `ldap3` package, installed dev-only and not wired into this project's CI; unlike the other four Slice 18 additions, this one does not currently run for real anywhere but a machine with `ldap3` installed by hand.
- Cross-detector authorization independence for this specific detector is inferred, not independently reconfirmed.

**Next steps:** a true end-to-end lab test, the same next step already recorded for every prior post-freeze detector; separately, a decision on whether to add `ldap3` to this project's locked CI dependencies so this real-network validation runs in CI like the other four Slice 18 additions do, and whether a simulated ldapjs-shaped crash-on-malformed-filter behavior is worth building to exercise the crash risk under test rather than only reasoning about it.
