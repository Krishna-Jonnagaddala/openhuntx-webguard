...
**Not done this slice.** No purpose-built vulnerable HTTP fixture with a real LDAP directory backend over real sockets exists for this detector yet.
...
**Proven:** the payload's structural reliability (an injected, unmatched closing parenthesis can only ever increase a filter's paren imbalance, never be absorbed by any surrounding template shape) was independently verified by hand-worked arithmetic across simple, wildcard-wrapped, and compound filter shapes and corroborated by real production incident reports, not merely asserted by analogy to SQLi; all 13 signatures are real, currently-accurate strings/class names from named, real libraries and servers, fact-checked against source and real bug reports before this detector was written, not assumed.

**Not Proven:** that this detector correctly fires against a real, genuinely vulnerable directory-backed HTTP server actually concatenating unescaped input into an LDAP filter; that it correctly abstains against a real, properly-escaping target; that it survives the full CLI/API/worker/executor pipeline; that any real-world target is actually detectable by it.

**Remaining risks:**
- On an ldapjs-backed Node.js target, this detector's own diagnostic payload can crash the target process, a materially different and larger risk than any other active detector in this project carries. An operator should be aware of this before authorizing this specific check against a target that might be running that library.
...
- No real-network or true end-to-end test exists yet, the same gap path traversal, command injection, XXE, and open redirect all have.
- Cross-detector authorization independence for this specific detector is inferred, not independently reconfirmed.

**Next steps:** a purpose-built vulnerable/safe local fixture (ideally including a simulated ldapjs-shaped crash-on-malformed-filter behavior, so the risk named above is exercised under test rather than only reasoned about) and a true end-to-end lab test, the same next step already recorded for every prior post-freeze detector.
