...
- The unscoped `registration_for()` is replaced by
  `get_registration(token_value, *, organization_id)`, which fails closed
...
`DatabaseMigrationError` (undefined table/column, a schema mismatch,
...
and there is no default that could silently activate SQLite, an
