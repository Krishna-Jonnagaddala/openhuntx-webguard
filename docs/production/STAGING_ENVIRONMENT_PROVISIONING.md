...
- **Postgres version and runtime roles**: PostgreSQL 16.4 (the same version pinned in `postgres.tf` and in `infra/compose/compose.postgres.yml`'s `postgres:16.10-alpine` — the 16.x minor-version difference between CI's Docker image and RDS's engine version is worth noting explicitly and is not expected to matter for RLS/GRANT semantics, but should be confirmed, not assumed, during validation). Runtime roles are exactly the seven created by `tenant_isolation_roles.sql` — no new role is introduced by staging itself. The bastion's own connection during validation uses the RDS master credential throughout, for bootstrap SQL, migrations, and as the test suite's own admin DSN — this mirrors CI's own `postgresql-integration` job exactly: its disposable Docker container likewise has one `webguard` role for everything (`.github/workflows/ci.yml`'s `WEBGUARD_DATABASE_URL`/`WEBGUARD_POSTGRES_TEST_DSN` are the identical connection string). The seven least-privilege roles are what this whole exercise validates, exercised *by* the test suite (which internally connects as each of them in turn to prove isolation); they are not the connection the test suite itself runs under.
