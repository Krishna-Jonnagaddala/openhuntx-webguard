...
Per this slice's explicit instruction, **the active permit-signing algorithm is not changed here**, regardless of which option below is selected. `LocalDevelopmentSigner`/Ed25519 remains the only wired signer in every environment today. This document makes the *decision* for the next slice's scoped, reviewed, regression-tested implementation work. It does not implement it.
...
## 2026-09-23 addendum: Option C's stated blocker is now closed, changing the calculus for this specific release

Option C above lists its blocker plainly: "requires an actual cryptographic-format migration... it still needs its own dedicated review slice." That work is now done, independent of anything in this document: `SUPPORTED_TRUSTSCAN_SIGNATURE_ALGORITHMS` has carried both `"Ed25519"` and `"ECDSA_SHA_256"` since the P1-7 slice (`scan_permits.py:34`), `_verify_ecdsa_sha256` is a real verifier (`signing.py`), and `test_p1_7_signature_algorithm_metadata.py` already proved a genuine KMS-backed ECDSA sign-verify round trip end to end through `TrustScanSigner`. `SigningKeyRegistry` already supports holding an Ed25519 key and an ECDSA_SHA_256 key simultaneously with zero changes (`add_verification_key`/`verify_by_key_id` resolve purely by `key_id`, never assume one algorithm for the whole registry). `production_startup.py`/`cli.py` already construct a real `KmsSigningProvider(boto3.client("kms"), key_id=...)` end to end when `WEBGUARD_SIGNING_PROVIDER=kms` is set (`cli.py:289`, `production_startup.py:231-232`).

One real gap was found and closed this session, not previously covered by any test: verification never checked that a permit's or receipt's self-reported `signature_algorithm` field actually matched the algorithm its `signing_key_id` was registered under. The cryptographic check itself was never affected (`VerificationKey.verify` always dispatches on the *registered* algorithm, never the self-report, so no downgrade attack existed), but a self-report that lied about which algorithm was actually used would previously pass silently. Fixed in `TrustScanSigner.verify()`/`verify_safety_receipt()` (`permits.py`): after a successful cryptographic verification, the resolved key's registered algorithm is now compared against the signed object's own declared algorithm, and a mismatch is rejected closed (`trustscan_permit_signature_algorithm_mismatch` / `trustscan_safety_receipt_signature_algorithm_mismatch`). Proven by 3 new tests in `test_p1_7_signature_algorithm_metadata.py`: an Ed25519-signed permit relabeled as `ECDSA_SHA_256`, a KMS/ECDSA-signed permit relabeled as `Ed25519`, and the equivalent safety-receipt case, are all rejected; the existing genuine dual-algorithm round-trip tests are unaffected.

**What this changes for the "recommended v1 production path" above, for this specific release only**: Option A (CloudHSM) still needs real hardware procurement (a cluster, a crypto-officer/crypto-user ceremony, ongoing cost) this release's own scope does not include. Option C now costs zero additional engineering, since its stated blocker is closed. For a first release specifically, `docs/production/WEBGUARD_RELEASE_CHECKLIST.md` selects Option C (`kms`) as the practical, hardware-free path, without overturning this document's own longer-term Option A recommendation for whenever CloudHSM procurement is in scope. Nothing here claims KMS signing has been run against real AWS: every proof above uses a duck-typed KMS client performing genuine cryptography in-process, never a real `boto3` call. That remains the one thing only a real AWS account can prove.
