...
## 2026-09-24 addendum: `api-sidecar.conf` actually run against a real backend, not just read

The previous addendum above shipped the config unvalidated ("no nginx binary was available"). This one closes that specific gap using `nginx:1.27-alpine` in a local Docker container:

- `nginx -t` against the exact, unmodified committed file (with a locally-generated self-signed cert dropped at the exact referenced path, `/etc/ssl/origin/api.openhuntx.com.pem`, since nginx's config test resolves certificate file paths, not just parses syntax): **passes clean**.
- A real `webguard-api serve --environment lab` process was started on `127.0.0.1:8765`, exactly as this sidecar expects. The container reached it via `host.docker.internal` rather than a literal `127.0.0.1:8765`, the one substitution made anywhere in this exercise (Docker Desktop for macOS puts a container in its own network namespace, so a container's own `127.0.0.1` is not the Mac host's; on a real single-host Linux deployment, where the sidecar and the API process share one real network namespace, this substitution does not apply and `127.0.0.1:8765` is exactly correct as committed). With that one substitution, end to end through the running sidecar: HTTP on port 80 redirects to HTTPS (`301`, correct `Location`); TLS terminates and `/healthz`/`/ready` proxy through to the real process (`200`, real JSON body); a real `POST /v1/auth/register` against the real API succeeds (`201`) with both `wg_session` (`HttpOnly`) and `wg_csrf` (not `HttpOnly`) cookies passing through the proxy hop unmodified, exactly matching the double-submit CSRF pattern section 8 above documents; a request with `Content-Length` over the 128k cap is rejected with `413` by nginx itself, never reaching the backend; killing the backend produces a generic `502` with no internal detail leaked.
- One real, if minor, gap this exercise found and fixed: every response, including nginx's own `502`/`413` error pages, carried `Server: nginx/1.27.5`, disclosing the exact nginx version to anyone who asks. `server_tokens off;` is now set in both server blocks; re-verified after the fix (`nginx -t` still passes; the `Server` header now reads bare `nginx`, no version).
- The `CF-Connecting-IP`/`X-Forwarded-For`/`X-Forwarded-Proto` forwarding directives were confirmed on the wire (a stand-in header-echoing backend in place of the real API showed exactly what nginx sends), separately from `_resolve_client_ip`'s own trust-gating logic in `http_api.py`, which `tests/unit/test_trusted_proxy.py` already covers directly at the unit level; this container exercise is what nginx itself does with those headers, not a repeat of that existing test.

Not covered by this, stated plainly: the real Cloudflare-issued origin certificate (a self-signed stand-in was used here, proving the TLS-termination mechanics, not certificate provenance), the real `api.openhuntx.com` DNS record, and a genuinely shared-host loopback peer address for `WEBGUARD_TRUSTED_PROXY_CIDRS` (this container topology's peer address is a Docker bridge gateway, not `127.0.0.1`, which is a property of testing nginx-in-a-container on macOS, not of the real deployment). All three require a deployed environment (M18-adjacent) to verify beyond what a local container can prove.
