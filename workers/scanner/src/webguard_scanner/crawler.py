...
class CrawlExecution:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            raise CrawlPolicyError(
                "crawl_pages_invalid",
                "pages must contain at least one CrawlPageRecord.",
            )

        if self.pages[0].url != root_url:
            raise CrawlPolicyError(
...
def crawl_same_origin(
    ...
    *,
    ...
    """Fetch a bounded breadth-first set of same-origin HTML pages.
    ...
    while queue and len(pages) < crawl_policy.maximum_pages:
        ...
        if (
            pages
            and crawl_policy.minimum_delay_seconds > 0
        ):
            time.sleep(
