...
# P1-2 Phase H: claim_next_leased now runs entirely through
# webguard_control.claim_next_job under worker_tenant_data (see
# postgres_pool.py's role_scoped_connection and postgres_jobs.py's own
# method), so this real build_production_components-backed E2E needs
# the same role/ACL/function/runtime-grant bootstrap
# tests/contract/test_identity_repository_contract.py's own setUpClass
# applies, in the same order: exactly what a real deployment must
# also apply before this code ever runs against it.
