...
# P1-2 Phase H: authenticate_session now runs its pre-auth resolve
# step under api_tenant_data (see postgres_pool.py's
# role_scoped_connection and postgres_sessions.py's own method), so
# this test needs the same role/ACL/function/runtime-grant bootstrap
