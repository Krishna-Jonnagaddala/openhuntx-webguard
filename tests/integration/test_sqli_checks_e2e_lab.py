"""One true end-to-end test of the SQLi active-detection feature:

CLI permit issuance (--active-check active.sqli.error) -> HTTP API job
submission -> real worker claiming the job -> real ScanJobExecutor ->
active-detector registry lookup -> the error-based SQLi detector -> a
CWE-89 NormalizedFinding merged into the persisted report, read back
through the HTTP API's job-result endpoint.

Uses a purpose-built, clearly-labeled local vulnerable-SQL fixture --
...
test_active_checks_e2e_lab.py (the reflected-XSS true E2E test); see that
file's module docstring for why each mock exists. This file focuses its
comments on what's SQLi-specific rather than repeating that rationale.
"""
...
class _SqliFixtureHandler(BaseHTTPRequestHandler):
    """A self-submitting search form at "/" whose "id" parameter is
    string-concatenated into a real SQLite query -- genuinely vulnerable,
    for this one E2E proof only. See test_sqli_error_detector_live.py for
    the fuller fixture with safe/broken/db-looking-text controls."""
    ...
    def do_GET(self) -> None:  # noqa: N802
        ...
        if "id" not in query:
            ...
            return
        value = query["id"][0]
        sql = f"SELECT id, name FROM products WHERE id = {value}"
        try:
            with self._lock:
        ...
        .add_extension(
            x509.SubjectAlternativeName(
                [
                    ...
                    x509.DNSName("sqli-e2e-lab-fixture.test"),
                ]
            ),
            critical=False,
        )
...
class SqliChecksEndToEndLabTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        ...
        cls.fixture_server = ThreadingHTTPServer(
            ("127.0.0.1", 0), _SqliFixtureHandler
        )
        ...
        cls.target = f"https://sqli-e2e-lab-fixture.test:{cls.fixture_port}/"
    ...
    def _validated_target(self) -> ValidatedTarget:
        return ValidatedTarget(
            ...
            scheme="https",
            hostname="sqli-e2e-lab-fixture.test",
    ...
    def test_cli_to_report_end_to_end_sql_injection(self) -> None:
        with TemporaryDirectory() as directory:
            ...
            now = datetime.now(timezone.utc)
            authorization_id = "a1b2c3d4-e5f6-4789-8abc-def012345678"
            auth_dir.mkdir(parents=True, exist_ok=True)
            write_owned_target_authorization_file(
                OwnedTargetAuthorization(
                    authorization_id=authorization_id,
                    organization="SQLi E2E Lab Org",
                    authorized_by="SQLi E2E Lab Operator",
                    target=self.target,
                    allowed_hosts=("sqli-e2e-lab-fixture.test",),
                    issued_at=now - timedelta(days=1),
                    expires_at=now + timedelta(days=300),
                    purpose="True end-to-end SQLi active-detection validation",
            ...
            self.assertIn(
                "Active checks authorized: active.sqli.error", permit_text
            )
                ...
                try:
                    ...
                    connection.request(
                        ...
                        headers={
                            ...
                            "Idempotency-Key": "e2e-active-sqli-1",
                        },
                    )
            ...
            self.assertTrue(
                active_findings,
                f"expected an active SQLi finding; report findings: "
                f"{[f.identity.rule_id for f in report.findings]}",
            )
            finding = active_findings[0]
            self.assertEqual(
                [i.value for i in finding.identifiers], ["CWE-89"]
            )
            self.assertTrue(
                finding.identity.rule_id.startswith("active.sqli.error")
            )
