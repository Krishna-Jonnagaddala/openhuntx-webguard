"""One true end-to-end test of the OS command-injection active-detection
feature:

CLI permit issuance (--active-check active.cmdi.marker) -> HTTP API job
submission -> real worker claiming the job -> real ScanJobExecutor ->
active-detector registry lookup -> the command-injection marker detector
-> a CWE-78 NormalizedFinding merged into the persisted report, read back
through the HTTP API's job-result endpoint.

Uses a purpose-built, clearly-labeled local vulnerable-shell fixture --
explicitly NOT Juice Shop and NOT a public site. Structurally this is the
same chain, scaffolding, and narrowly-justified mocks as
test_sqli_checks_e2e_lab.py (see that file's module docstring for why
each mock exists: validate_target_url pointed at the local fixture,
owned_target._public_addresses skipping the public-only filter for the
local fixture address, and ssl.create_default_context trusting a
freshly generated self-signed certificate for the fixture). This file
focuses its comments on what's command-injection-specific rather than
repeating that rationale.
"""
...
class _CmdiFixtureHandler(BaseHTTPRequestHandler):
    """A self-submitting form at "/" whose "cmd" parameter is
    interpolated, unsanitized, into a real shell command line -- the
    same mechanism test_command_injection_detector_live.py's own
    "/vulnerable" route already proved runs a real shell on this
    machine. Doing double duty on "/" (discovery form with no query
    parameter, real vulnerable action once "cmd" is present) matches
    how the SQLi and XSS single-page e2e lab fixtures both use their
    one root route."""
    ...
    def do_GET(self) -> None:  # noqa: N802
        ...
        if "cmd" not in query:
            body = (
                b"<html><body>"
                b'<form method="GET" action=""><input name="cmd"></form>'
                b"</body></html>"
            )
            self._respond(body)
            return
        ...
        try:
            ...
            self._respond(f"<html>{stdout}</html>".encode())
        ...
        .add_extension(
            x509.SubjectAlternativeName(
                [
                    ...
                    x509.DNSName("cmdi-e2e-lab-fixture.test"),
                ]
            ),
            critical=False,
        )
...
class CommandInjectionChecksEndToEndLabTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        ...
        cls.fixture_server = ThreadingHTTPServer(
            ("127.0.0.1", 0), _CmdiFixtureHandler
        )
        ...
        cls.target = f"https://cmdi-e2e-lab-fixture.test:{cls.fixture_port}/"
    ...
    def _validated_target(self) -> ValidatedTarget:
        return ValidatedTarget(
            ...
            scheme="https",
            hostname="cmdi-e2e-lab-fixture.test",
    ...
    def test_cli_to_report_end_to_end_command_injection(self) -> None:
        with TemporaryDirectory() as directory:
            ...
            write_owned_target_authorization_file(
                OwnedTargetAuthorization(
                    ...
                    organization="Cmdi E2E Lab Org",
                    authorized_by="Cmdi E2E Lab Operator",
                    target=self.target,
                    allowed_hosts=("cmdi-e2e-lab-fixture.test",),
                    issued_at=now - timedelta(days=1),
                    expires_at=now + timedelta(days=300),
                    purpose="True end-to-end command-injection active-detection validation",
            ...
            self.assertIn(
                "Active checks authorized: active.cmdi.marker", permit_text
            )
                ...
                try:
                    ...
                    connection.request(
                        ...
                        headers={
                            ...
                            "Idempotency-Key": "e2e-active-cmdi-1",
                        },
                    )
            ...
            self.assertTrue(
                active_findings,
                f"expected an active command-injection finding; report "
                f"findings: {[f.identity.rule_id for f in report.findings]}",
            )
            finding = active_findings[0]
            self.assertEqual(
                [i.value for i in finding.identifiers], ["CWE-78"]
            )
            self.assertTrue(
                finding.identity.rule_id.startswith("active.cmdi.marker")
            )
