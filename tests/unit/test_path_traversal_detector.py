"""Unit tests for the active path-traversal detector.

Covers the classification logic (baseline vs. diagnostic comparison,
/etc/passwd content-signature matching), false-positive resistance, and
the same-origin/budget/cancellation safety boundaries already proven for
the SQL-injection detector, against a fake connection so no network
access is required.
"""
...
def _candidate(url: str = "http://example.com/download") -> DetectionCandidate:
    return DetectionCandidate(url=url, parameter="file")
...
class PathTraversalClassificationTests(unittest.TestCase):
    def test_genuine_passwd_disclosure_with_status_change_is_confirmed(
        self,
    ) -> None:
        def responder(value):
            ...
            return _FakeResponse(b"<html>report.pdf</html>", status=404)
        ...
        self.assertEqual([i.value for i in finding.identifiers], ["CWE-22"])
        self.assertEqual(result.records[0].outcome, PathTraversalOutcome.CONFIRMED)

    def test_disclosure_without_status_change_is_probable(self) -> None:
        def responder(value):
            if value == _TRAVERSAL_PAYLOAD:
                return _FakeResponse(
                    b"<html>root:!:0:0:root:/root:/bin/bash</html>", status=200
                )
            return _FakeResponse(b"<html>report.pdf</html>", status=200)

        result = _run(_ScriptedConnection(responder))
        self.assertEqual(len(result.findings), 1)
        self.assertEqual(result.findings[0].confidence.value, "high")
        self.assertEqual(result.records[0].outcome, PathTraversalOutcome.PROBABLE)

    def test_generic_404_with_no_signature_is_inconclusive(self) -> None:
        def responder(value):
            return _FakeResponse(b"<html>Not Found</html>", status=404)

        result = _run(_ScriptedConnection(responder))
        self.assertEqual(result.findings, ())
        self.assertEqual(result.records[0].outcome, PathTraversalOutcome.INCONCLUSIVE)

    def test_reflected_probe_string_without_file_content_is_inconclusive(
        self,
    ) -> None:
        def responder(value):
            return _FakeResponse(
                f"<html>File not found: {value}</html>".encode(), status=404
            )
    ...
    def test_passwd_looking_text_already_in_baseline_is_inconclusive(
        self,
    ) -> None:
        # The exact same signature appears whether or not the probe is
        # sent: pre-existing content, not evidence of traversal.
        def responder(value):
            return _FakeResponse(
                b"<html>Example config: root:x:0:0: (documentation)</html>",
                status=200,
            )
    ...
    def test_multiple_parameters_each_classified_independently(self) -> None:
        def responder(value):
            if value == _TRAVERSAL_PAYLOAD:
                return _FakeResponse(b"root:x:0:0:root:/root:/bin/bash", status=200)
            return _FakeResponse(b"<html>report.pdf</html>", status=404)
...
class PathTraversalSafetyBoundaryTests(unittest.TestCase):
    ...
    def test_cancellation_before_first_candidate_probes_nothing(
        ...
        with patch(
            ...
            result = run_path_traversal_detector(
                _target(),
                (_candidate(),),
                _context(),
